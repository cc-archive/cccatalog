...
CLEANUP_BUFFER_SIZE = int(os.environ.get('DB_BUFFER_SIZE', 100000))

TABLE_NAME_STUB = 'provider_image_data'
...
def _clean_data_worker(
    ...
    worker_postgres = PostgresHook(postgres_conn_id=postgres_conn_id)
    worker_conn = worker_postgres.get_conn()
...
def clean_image_data(
    ...
    start_time = time.time()
    table = _get_table_name(identifier)
...
def _get_table_name(
        identifier,
        table_name_stub=TABLE_NAME_STUB,
):
    return f'{table_name_stub}{identifier}'
